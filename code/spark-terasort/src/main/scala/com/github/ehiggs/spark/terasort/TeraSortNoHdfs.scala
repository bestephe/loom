...
object TeraSortNoHdfs {
  def main(args: Array[String]) {

    if (args.length < 1) {
      println("Usage:")
      println("DRIVER_MEMORY=[mem] spark-submit " +
        "com.github.ehiggs.spark.terasort.TeraSortNoHdfs " +
    ...
    val conf = new SparkConf()
      .setAppName(s"TeraSortNoHdfs ($size)")
    ...
    val sorted = dataset.partitionBy(new TeraSortPartitioner(dataset.partitions.size))
    //println("Classname:" + sorted.getClass.getSimpleName)
