...
def gen_cdf(data_points):
    counts, edges = np.histogram(data_points, bins=5000, normed=True)
...
def main():
    ...
    parser.add_argument('--sq-pfifo', help='The SQ PFIFO files to parse.',
        nargs='+', required=True)
    parser.add_argument('--sq-sfq', help='The SQ SFQ files to parse.',
        nargs='+', required=True)
    parser.add_argument('--mq-pfifo', help='The MQ PFIFO files to parse.',
        nargs='+', required=True)
    parser.add_argument('--mq-sfq', help='The MQ SFQ files to parse.',
        nargs='+', required=True)
    # TODO: what percentile to take a CDF of.  Currently only uses average.
    parser.add_argument('--outf', help='The output file.')
    args = parser.parse_args()

    files_and_names = [(args.sq_pfifo, 'SQ-PFIFO'), (args.sq_sfq, 'SQ-SFQ'),
        (args.mq_pfifo, 'MQ-PFIFO'), (args.mq_sfq, 'MQ-SFQ')]
    lines = []
