...
def opps_to_gbps(opps):
    gbps = opps * OP_SIZE * 8 / 1e9
    return gbps

def gbps_to_opps(gbps):
    ops_persec = gbps * 1e9 / 8 / OP_SIZE
    return ops_persec
...
def memcached_ycsb_run(args):
    ...
    run_cmd_tmpl = 'cd %(ycsb_dir)s; %(prefix)s ./bin/ycsb run memcached -s ' \
        '-P %(workload)s -P %(properties)s -threads 16 -target %(target)d ' \
        '-p status.interval=1'
    if args.high_prio:
        prefix = 'sudo cgexec -g net_prio:high_prio '
    else:
        prefix = ''
    target = int(gbps_to_opps(TARGET_GBPS))
    run_cmd_args = {
        'ycsb_dir': YCSB_DIR, 
        'workload': os.path.abspath(WORKLOAD),
        'properties': os.path.abspath(PROPERTIES),
